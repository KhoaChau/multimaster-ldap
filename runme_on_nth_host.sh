...
#check if the rootCA exist
if [ ! -f "certs/rootCA.crt" ]; then
    echo ""
    echo "rootCA files does not exist. Please copy it over from the 1st host. Exiting..."
    echo "scp 1stHOSTIP:/tmp/root* certs/."
    echo ""
    exit 1
fi
...
#now we can start copying and editing files going fwd
cp docker-compose-template.yml docker-compose.yml
cp lam-template.conf lam/data/config/lam.conf

#filling in your docker-compose file
